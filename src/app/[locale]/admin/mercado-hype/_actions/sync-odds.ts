...
import { conditions, markets, outcomes } from '@/lib/db/schema/events/tables'
...
export async function syncSingleMarketAction(polyId: string) {
  try {
    ...
    if (match.length > 0) {
      ...
      if (prices.length >= 2) {
        // Update Sim
        await db.update(outcomes).set({ buy_price: Number(prices[0]).toFixed(2), updated_at: new Date() } as any).where(and(eq(outcomes.condition_id, condId), eq(outcomes.outcome_index, 0)))

        // Update Nao
        await db.update(outcomes).set({ buy_price: Number(prices[1]).toFixed(2), updated_at: new Date() } as any).where(and(eq(outcomes.condition_id, condId), eq(outcomes.outcome_index, 1)))
