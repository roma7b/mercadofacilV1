...
export function LiveChat({ events, className }: LiveChatProps) {
  ...
  return (
    <div className={cn(`
      flex h-full flex-col border-l border-border bg-card/50 backdrop-blur-sm transition-all duration-300
    `, className)}
    >
      <div className="flex items-center justify-between border-b border-border bg-card/80 px-5 py-4">
        ...
        <span className="flex items-center gap-1.5 text-2xs font-bold text-emerald-500 tabular-nums">
      ...
      <div ref={scrollRef} className="custom-scrollbar flex-1 space-y-3 overflow-y-auto p-4">
        {loading
          ...
          : messages.length === 0
            ? (
                <div className="
                  flex h-full items-center justify-center px-6 text-center text-xs text-muted-foreground italic
                "
                >
            ...
            : (
                messages.map(m => (
                  <div key={m.id} className="animate-in text-xs/relaxed duration-300 fade-in slide-in-from-bottom-2">
                    <span className={cn(
                      'mr-1.5 font-bold tracking-tight',
                      m.isBot ? m.color : getRandomColor(m.user_id),
                    )}
                    >
                      {m.users?.username || 'Usuário'}
                      {m.user_id === user?.id && ' (você)'}
                      :
                    </span>
                    <span className="font-medium wrap-break-word text-foreground/90">
                      {m.mensagem}
                      {m.isBot && m.previsoes && (
                        <span className="ml-1.5 text-[9px] font-bold text-muted-foreground opacity-60">
                          —
                          {' '}
                          {m.previsoes}
                          {' '}
      ...
      <div className="border-t border-border bg-card/80 p-4">
        {!user
          ? (
              <div className="w-full rounded-xl border border-border bg-secondary/50 px-4 py-5 text-center">
                <p className="mb-3 text-[11px] font-medium text-muted-foreground">Faça login para participar do chat</p>
                <Button
                  variant="default"
                  size="sm"
                  className="w-full text-2xs font-bold tracking-widest uppercase"
              ...
              <form onSubmit={handleSend} className="relative flex flex-col gap-2">
                <div className="relative">
                  <input
                    ...
                    maxLength={200}
                    className="
                      w-full rounded-full border border-border bg-background/50 py-3.5 pr-12 pl-5 text-xs font-medium
                      text-foreground shadow-inner transition-all
                  ...
                  <button
                    ...
                    className="
                      absolute top-1/2 right-2 flex size-8 -translate-y-1/2 items-center justify-center
