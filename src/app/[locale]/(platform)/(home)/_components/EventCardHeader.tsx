      ...
      <IntentPrefetchLink href={eventHref} className="flex flex-1 items-center gap-2 pr-2">
        ...
        >
          {(event.slug.startsWith('live_') && event.livestream_url && event.livestream_url.includes('.m3u8') && !event.slug.includes('btc')) && (
            <span className="
              mr-1.5 mb-0.5 inline-flex items-center rounded-sm border border-red-500/20 bg-red-500/15 px-1 pb-px
              align-middle text-2xs font-bold tracking-wider text-red-500 uppercase
            "
            >
              <span className="mr-1 size-1.5 animate-pulse rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]">
              </span>
              LIVE
            </span>
          )}
