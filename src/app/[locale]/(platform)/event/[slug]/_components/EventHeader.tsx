...
function resolveEventHeaderTaxonomy({
  ...
  const normalizedEventTags = (event.tags ?? [])
    ...
    .map(tag => ({
      isMainCategory: tag.isMainCategory ?? (tag as any).is_main_category ?? false,
      label: (tag.name ?? '').trim(),
...
export default function EventHeader({ event }: EventHeaderProps) {
  ...
  return (
    <div
      className={cn(
        'relative z-10 flex items-center gap-3 transition-all ease-in-out',
        { 'sticky top-26 -mx-4 translate-y-1 bg-background px-4 py-3 pr-6 md:translate-y-3 lg:top-28 lg:translate-y-1': scrolled },
      )}
    >
      {scrolled && (
